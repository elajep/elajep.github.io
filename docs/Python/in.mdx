...
```js title="/src/pages/helloReact.js"
...
<TwoColumns centerLeft>

<div>
  By default, any Markdown or JavaScript file starting with `_` will be ignored
  and no routes will be created for that file (see the `exclude` option).
